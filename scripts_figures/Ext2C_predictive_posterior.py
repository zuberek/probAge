...
plt.scatter(site_data.var.age, site_data.X.T, label='Data', color='tab:grey', alpha=0.05, linewidth=0, s=15)

# Plot the 95% predictive interval
plt.plot(ys_mean[:,0], ys_mean[:,1], label='Mean', color=colors[1], alpha=0.5)
plt.plot(ys_upper[:,0], ys_upper[:,1], label='95% CI', color=colors[1], dashes=[5,3], alpha=0.5)
