...
for i, n_sites in enumerate(tqdm(n_sites_grid)):
    maps = modelling.vectorize_all_participants(amdata[:n_sites])
    accs[i] = maps['acc']
    biases[i] = maps['bias']
...
sns.lineplot(x=n_sites_label, y=r2_array, marker='o', color='orange', label='r2 value', ax=ax11)
...
sns.lineplot(x=n_sites_label, y=r2_array, marker='o', color='orange', label='r2 value', ax=ax22)