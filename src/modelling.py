...
def linear_sites(amdata, compute_MAP=False, compute_trace=True, cores=CORES, show_progress=False):
...
def bio_sites(amdata, compute_MAP=False, compute_trace=True, show_progress=False, init_nuts='auto', target_accept=0.9, cores=CORES):
...
def bio_fit(amdata, show_progress=False, cores=CORES):
    ROUND = 7
    trace_bio = bio_sites(amdata, compute_trace=True, compute_MAP=False, cores=cores, show_progress=show_progress)['trace']
    bio_fit = az.summary(trace_bio, round_to=ROUND)
    bio_fit.index = pd.MultiIndex.from_tuples([(index_tuple[1][:-1], 'bio', index_tuple[0]) for index_tuple in bio_fit.index.str.split('[')],
                            names=['site', 'model', 'param'])
...
def site_offsets(amdata, normal=True, compute_MAP=True, compute_trace=False, show_progress=False,
                         map_method='L-BFGS-B'):
